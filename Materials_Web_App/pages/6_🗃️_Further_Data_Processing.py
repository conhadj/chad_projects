...
def scale_and_pca(df):
    ...
    # Check if 'rating' and 'Su/Sy' exist in the DataFrame and add them to the features list
    if 'rating' in df.columns:
        features.append('rating')
    if 'Su/Sy' in df.columns:
        features.append('Su/Sy')
    # Scale the features
...
def further_processing():
    ...
    if st.button('Scale/Normalize Features'):
        df = scale_features(df)
        st.session_state['df'] = df  # Save the updated dataframe back to session state
        st.success("Features have been scaled/normalized.")
    ...
    if st.checkbox('Resample Data with SMOTE'):
        ...
        st.session_state['df'] = resampled_df  # Save the updated dataframe back to session state
